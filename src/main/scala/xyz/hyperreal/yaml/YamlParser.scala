...
class YamlLexical
  ...
  override def token: Parser[Token] =
    anchorToken | aliasToken | scalarToken | super.token

  override def identChar: Parser[Char] = letter | elem('_') // | elem('$')
  ...
  private def anchorToken: Parser[Token] =
    '&' ~> rep1(elem("anchor", c => c.isLetterOrDigit)) ^^ (l =>
      Anchor(l.mkString))
  ...
  private def scalarToken: Parser[Token] =
    '\'' ~> rep(
      ('\'' ~ '\'' ^^^ "''") |
        (guard(not('\'')) ~> elem("", _ => true))) <~ '\'' ^^ { l =>
      StringLit(quoteEscape(newlines(l mkString)))
    } |
      '"' ~> rep(guard(not('"')) ~> (('\\' ~ '"' ^^^ "\\\"") | elem(
        "",
        _ => true))) <~ '"' ^^ { l =>
        StringLit(escape(newlines(l mkString)))
      } |
      ...
      if (!r.atEnd) {
        if (r.first == '\\') {
          ...
          if (r.rest.first == 'u') {
            ...
            buf append Integer
              .valueOf(new String(Array(nextc, nextc)), 16)
              .toChar
            chr(u)
          } else {
            r.rest.first match {
...
class YamlParser extends StandardTokenParsers with PackratParsers {
  ...
  def parse(src: io.Source): AST =
    parse(new PagedSeqReader(PagedSeq.fromSource(src)))

  import lexical.{Alias, Anchor, Dedent, Indent, Newline, TextLit}
  ...
  lazy val pos
    : PackratParser[Position] = positioned(success(new Positional {})) ^^ {
    _.pos
  }
  ...
  lazy val primitive: PackratParser[PrimitiveAST] =
    opt(anchor) ~ stringLit ^^ { case a ~ s => StringAST(a, s) } |
      opt(anchor) ~ textLit ^^ {
        ...
        case a ~ ("true" | "True" | "TRUE")       => BooleanAST(a, v = true)
        case a ~ ("false" | "False" | "FALSE")    => BooleanAST(a, v = false)
        case a ~ (".inf" | "+.inf" | ".Inf" | "+.Inf" | ".INF" | "+.INF") =>
          NumberAST(a, Double.PositiveInfinity)
        case a ~ ("-.inf" | "-.Inf" | "-.INF") =>
          NumberAST(a, Double.NegativeInfinity)
